...
# Установка бэкенда должна быть сразу после импорта matplotlib
matplotlib.use("Agg")
...
def reshape_vector(vec: np.ndarray) -> np.ndarray:
    """Изменяет форму массива из 10 элементов в таблицу 2x5."""
...
if __name__ == "__main__":
    ...
    try:
        data_path = "numpy_lab/data/students_scores.csv"
        ...
        students_ids = np.arange(1, len(data) + 1)
        math_scores = data[:, 0]
        plot_line(students_ids, math_scores)

        print("Графики успешно сохранены в папку 'plots/'")
    except FileNotFoundError:
        print(f"Ошибка: Файл по пути {data_path} не найден.")
